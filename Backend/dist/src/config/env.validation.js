...
function validarVariablesEntorno() {
    ...
    if (faltantes.length > 0) {
        throw new Error(`❌ ERROR CRÍTICO: Variables de entorno faltantes: ${faltantes.join(', ')}\n` +
    ...
    if (process.env.JWT_SECRET === 'secreto_por_defecto' || process.env.JWT_SECRET === 'secret') {
        throw new Error('❌ ERROR CRÍTICO: JWT_SECRET no puede usar valores por defecto inseguros.\n' +
            'Por favor, configure una clave secreta segura en la variable JWT_SECRET.');
    }
    // Validar longitud mínima de JWT_SECRET
    if (process.env.JWT_SECRET && process.env.JWT_SECRET.length < 32) {
        console.warn('⚠️  ADVERTENCIA: JWT_SECRET es muy corto (menos de 32 caracteres). ' +
    ...
    for (const [grupo, variables] of Object.entries(variablesOpcionales)) {
        ...
        if (tieneAlguna && !tieneTodas) {
            ...
            throw new Error(`❌ ERROR: Si usa ${grupo}, debe configurar todas las variables: ${faltantes.join(', ')}`);
        }
    }
    console.log('✅ Variables de entorno validadas correctamente');
}
exports.default = validarVariablesEntorno;
