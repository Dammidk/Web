...
exports.reportesService = {
    ...
    async reportePorVehiculo(vehiculoId, fechaDesde, fechaHasta) {
        const vehiculo = await database_1.default.vehiculo.findUnique({ where: { id: vehiculoId } });
        if (!vehiculo)
        ...
        for (const v of viajes) {
            gastosViaticos += await gastos_repository_1.gastosRepository.sumarGastosViaje(v.id);
        }
        ...
        const gastosMantenimientos = await mantenimiento_repository_1.mantenimientosRepository.sumarCostos(vehiculoId, fechaDesde, fechaHasta);
    ...
    async reportePorChofer(choferId, fechaDesde, fechaHasta) {
        const chofer = await database_1.default.chofer.findUnique({ where: { id: choferId } });
        if (!chofer)
        ...
        const pagosRealizados = await pagosChofer_repository_1.pagosChoferRepository.sumarPagos(choferId, fechaDesde, fechaHasta);
        return {
            chofer,
            periodo: { desde: fechaDesde, hasta: fechaHasta },
    ...
    async reportePorCliente(clienteId, fechaDesde, fechaHasta) {
        const cliente = await database_1.default.cliente.findUnique({ where: { id: clienteId } });
        if (!cliente)
    ...
    async reporteMensualComparativo(anio, meses) {
        ...
        for (const mes of mesesConsulta) {
            ...
            const pagosChoferesDirectos = await pagosChofer_repository_1.pagosChoferRepository.sumarPagos(undefined, fechaInicio, fechaFin);
    ...
    async reporteCartera() {
        ...
        return Object.values(carteraPorCliente).sort((a, b) => b.totalDeuda - a.totalDeuda);
    },
    ...
    async reporteGeneral(fechaDesde, fechaHasta) {
        ...
        const mantenimientos = await mantenimiento_repository_1.mantenimientosRepository.sumarCostos(undefined, fechaDesde, fechaHasta);
