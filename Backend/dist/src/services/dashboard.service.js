...
exports.dashboardService = {
    async obtenerResumen() {
        ...
        const [vehiculosActivos, vehiculosTotal, choferesActivos, choferesTotal, clientesActivos, clientesTotal, materialesTotal, estadisticasViajes, topVehiculos, topClientes, alertas] = await Promise.all([
            vehiculo_repository_1.vehiculoRepository.countActivos(),
            vehiculo_repository_1.vehiculoRepository.countTotal(),
            chofer_repository_1.choferRepository.countActivos(),
            chofer_repository_1.choferRepository.countTotal(),
            cliente_repository_1.clienteRepository.countActivos(),
            cliente_repository_1.clienteRepository.countTotal(),
            material_repository_1.materialRepository.countTotal(),
            viajes_repository_1.viajesRepository.getEstadisticasMensuales(anioActual, mesActual),
            this.obtenerTopVehiculos(fechaInicio, fechaFin),
            this.obtenerTopClientes(fechaInicio, fechaFin),
            alertas_service_1.alertasService.obtenerAlertas()
        ]);
        ...
        for (const viaje of viajesCompletados) {
            gastosViaticos += await gastos_repository_1.gastosRepository.sumarGastosViaje(viaje.id);
        }
        ...
        const totalPagosChoferes = await pagosChofer_repository_1.pagosChoferRepository.sumarPagos(undefined, // todos los choferes
        fechaInicio, fechaFin);
        // 3. Gastos de mantenimientos (del mes)
        const gastosMantenimientos = await mantenimiento_repository_1.mantenimientosRepository.sumarCostos(undefined, fechaInicio, fechaFin);
        ...
        const ingresosDevengados = await database_1.default.viaje.aggregate({
            where: {
                estado: 'COMPLETADO',
                fechaLlegadaReal: { gte: fechaInicio, lte: fechaFin }
            },
        ...
        const ingresosCobrados = await database_1.default.viaje.aggregate({
            where: {
                estado: 'COMPLETADO',
                fechaLlegadaReal: { gte: fechaInicio, lte: fechaFin },
        ...
        const saldoPendienteHistorico = await database_1.default.viaje.aggregate({
            where: {
                estado: 'COMPLETADO',
        ...
        const totalPorCobrarHistorico = Number(saldoPendienteHistorico._sum.tarifa || 0) -
     ...
     */
    async obtenerTopVehiculos(fechaInicio, fechaFin) {
        const resultado = await database_1.default.viaje.groupBy({
            by: ['vehiculoId'],
            where: {
                estado: 'COMPLETADO',
                fechaLlegadaReal: { gte: fechaInicio, lte: fechaFin }
        ...
        const vehiculosConDatos = await Promise.all(resultado.map(async (r) => {
            const vehiculo = await database_1.default.vehiculo.findUnique({
     ...
     */
    async obtenerTopClientes(fechaInicio, fechaFin) {
        const resultado = await database_1.default.viaje.groupBy({
            by: ['clienteId'],
            where: {
                estado: 'COMPLETADO',
        ...
        const clientesConDatos = await Promise.all(resultado.map(async (r) => {
